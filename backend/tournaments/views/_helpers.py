...
# Plik udostępnia wspólne helpery widoków turniejowych niezwiązane bezpośrednio z definicją endpointów.
...
def public_access_or_403(request, tournament: Tournament) -> Optional[Response]:
    # Publiczny gate dopuszcza organizera, asystenta, uczestnika z preview lub publikację.
...
def _sync_two_leg_pair_winner_if_possible(stage: Stage, tournament: Tournament, match: Match) -> None:
    # Dwumecz synchronizuje zwycięzcę na obu rekordach pary.
    ...
    if any(item.status != Match.Status.FINISHED for item in group):
        return
...
def _soft_reset_downstream_for_team_change(
    *,
    ...
    # Reset czyści wyniki dalszych rund po zmianie zwycięzcy wcześniejszego etapu.
    downstream_matches = (
...
def _try_auto_advance_knockout(stage: Stage) -> None:
    # Auto-awans działa tylko po pełnym zamknięciu bieżącej rundy.
    tournament = stage.tournament
    ...
    if any(match.status != Match.Status.FINISHED or not match.winner_id for match in matches):
        return

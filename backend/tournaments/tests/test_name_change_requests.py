...
class TournamentTeamDivisionChangeRequestApiTests(TestCase):
    """Testy API zabezpieczają przepływ wniosków o zmianę dywizji uczestnika."""

    def setUp(self):
        self.organizer = create_test_user("division-change-owner@example.com")
        self.participant = create_test_user("division-change-participant@example.com")
        self.other_user = create_test_user("division-change-other@example.com")
        self.client = APIClient()
    ...
    def _division_change_request_model(self):
        return apps.get_model("tournaments", "DivisionChangeRequest")

    def _create_tournament_context(self):
        self.client.force_authenticate(user=self.organizer)

        response = self.client.post(
            "/api/tournaments/",
            {
                "name": "Turniej zmiany dywizji",
                "discipline": "football",
            },
            format="json",
        )
        ...
        tournament = Tournament.objects.get(name="Turniej zmiany dywizji")
        tournament.participants_public_preview_enabled = True
        tournament.save(update_fields=["participants_public_preview_enabled"])

        from_division = tournament.get_default_division()

        to_division = apps.get_model("tournaments", "Division").objects.create(
            tournament=tournament,
            name="Druga dywizja",
        ...
        Team.objects.bulk_create(
            [
                Team(tournament=tournament, division=to_division, name="Druga 1", is_active=True),
                Team(tournament=tournament, division=to_division, name="Druga 2", is_active=True),
            ]
        )
        ...
        team.name = "Uczestnik pierwszej dywizji"
        team.registered_user = self.participant
        team.save(update_fields=["name", "registered_user"])

        registration = self._registration_model().objects.create(
            tournament=tournament,
            division=from_division,
            user=self.participant,
            team=team,
            display_name="Uczestnik pierwszej dywizji",
        )

        return tournament, from_division, to_division, team, registration

    def _create_pending_request(self):
        tournament, from_division, to_division, team, registration = self._create_tournament_context()
        ...
        response = self.client.post(
            ...
            + f"?division_id={from_division.id}",
            {"target_division_id": to_division.id},
            format="json",
        )

        self.assertEqual(response.status_code, 201)

        request_model = self._division_change_request_model()
        division_request = request_model.objects.get(id=response.json()["request"]["id"])

        return tournament, from_division, to_division, team, registration, division_request

    def test_registered_participant_can_submit_division_change_request(self):
        tournament, from_division, to_division, team, registration = self._create_tournament_context()
        ...
        division_request = self._division_change_request_model().objects.get(tournament=tournament)

        self.assertEqual(division_request.registration_id, registration.id)
        self.assertEqual(division_request.team_id, team.id)
        self.assertEqual(division_request.requested_by_id, self.participant.id)
        self.assertEqual(division_request.from_division_id, from_division.id)
        self.assertEqual(division_request.to_division_id, to_division.id)
        self.assertEqual(division_request.status, self._division_change_request_model().Status.PENDING)

    def test_participant_cannot_submit_second_pending_division_change_request(self):
        tournament, from_division, to_division, _team, _registration, _division_request = self._create_pending_request()
    ...
    def test_unregistered_user_cannot_submit_division_change_request(self):
        tournament, from_division, to_division, _team, _registration = self._create_tournament_context()
        ...
        self.assertEqual(response.status_code, 403)
        self.assertFalse(self._division_change_request_model().objects.filter(requested_by=self.other_user).exists())

    def test_organizer_can_list_pending_division_change_requests(self):
        tournament, from_division, _to_division, _team, _registration, division_request = self._create_pending_request()
        ...
        self.assertEqual(len(items), 1)
        self.assertEqual(items[0].get("id"), division_request.id)

    def test_organizer_can_approve_division_change_request(self):
        tournament, from_division, to_division, old_team, registration, division_request = self._create_pending_request()
        ...
        response = self.client.post(
            self._api_url_for_view(
                ...
                pk=tournament.id,
                request_id=division_request.id,
            )
        )

        self.assertEqual(response.status_code, 200)

        division_request.refresh_from_db()
        registration.refresh_from_db()
        old_team.refresh_from_db()

        self.assertEqual(division_request.status, self._division_change_request_model().Status.APPROVED)
        self.assertEqual(division_request.decided_by_id, self.organizer.id)
        self.assertEqual(registration.division_id, to_division.id)
        self.assertIsNotNone(registration.team_id)

        new_team = Team.objects.get(pk=registration.team_id)

        self.assertEqual(new_team.division_id, to_division.id)
        self.assertEqual(new_team.registered_user_id, self.participant.id)
        self.assertEqual(new_team.name, "Uczestnik pierwszej dywizji")
        self.assertIsNone(old_team.registered_user_id)

    def test_organizer_can_reject_division_change_request(self):
        tournament, from_division, _to_division, old_team, registration, division_request = self._create_pending_request()
        ...
        self.assertEqual(division_request.status, self._division_change_request_model().Status.REJECTED)
        self.assertEqual(registration.division_id, from_division.id)
        self.assertEqual(registration.team_id, old_team.id)
        self.assertEqual(old_team.registered_user_id, self.participant.id)
